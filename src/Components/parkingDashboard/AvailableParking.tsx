...
const AvailableParking = ({ data, parkedDataLength }: { data: any, parkedDataLength: number }) => {
    ...
    return (
        <div className="flex">
            <div className="bg-gray-300 p-4 w-full flex justify-between border-b border-gray-700">
                ...
                {modalOpen && (
                    <ActivateParking onSubmit={handleButtonClick} onCancel={handleButtonClick} onClose={handleButtonClick}/>
                )}
                {/* <button className="bg-gray-400 p-3 rounded-md m-4" onClick={handleButtonClick}>Aktiver parkering</button> */}
                <button className="bg-gray-400 p-3 rounded-md m-4" onClick={() => setModalOpen(true)}>Aktiver parkering</button>
            </div>
        </div>
    );
}
